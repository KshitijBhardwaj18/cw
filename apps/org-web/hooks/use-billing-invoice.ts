...
export function useBillingInvoice() {
	...
	const [currentStatus, setCurrentStatus] = useState<string>(uiStatus);
	...
	const uploadSupportingDocuments = useCallback(async (files: File[]) => {
		if (!files.length) return undefined;
		const uploaded = await Promise.all(
			files.map((file) =>
				TimekeepingService.uploadDisputeSupportingDocument(file),
			),
		);
