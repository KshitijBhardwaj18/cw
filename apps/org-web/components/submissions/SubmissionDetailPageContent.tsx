		...
		<div className="space-y-6">
					...
					<CardContent className="flex flex-wrap items-end gap-3">
						<div className="min-w-[200px] flex-1 space-y-2">
							...
							{canUpdateCurrentStage ? (
								<Select
									value={hiringStage}
									onValueChange={(v) => setHiringStage(v as SubmissionStageKey)}
								>
			...
			<div className="grid gap-4 md:grid-cols-2">
				<Card>
					...
					<CardContent className="grid gap-4 sm:grid-cols-2">
						<UppercaseField label="Occupation" value={detail.occupationLabel} />
						<UppercaseField label="Department" value={detail.departmentName} />
				...
				<CardContent>
					<div className="space-y-4">
						...
						<div className="grid gap-4 sm:grid-cols-2">
							...
							<DetailItem label="Regional nurse" value={detail.regionalNurse} />
							<DetailItem
					...
					<CardContent className="space-y-6">
						{detail.occupationalQuestionnaire.map((row, i) => (
							<div key={`occ-q-${i}`} className="space-y-1">
								<p className="text-muted-foreground text-sm">{row.question}</p>
			...
			{detail.summaryNote?.trim() ? (
				<Card>
					...
					<CardContent>
						<p className="whitespace-pre-wrap text-sm">{detail.summaryNote}</p>
					</CardContent>
				</Card>
			) : null}
		</div>
