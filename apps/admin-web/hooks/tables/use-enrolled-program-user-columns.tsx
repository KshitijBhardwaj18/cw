...
import type { EnrolledProgramUserRow } from "@/types/users";
...
type Options = {
	onSendInvite?: (row: EnrolledProgramUserRow) => void;
	onRemove?: (row: EnrolledProgramUserRow) => void;
};

export function useEnrolledProgramUserColumns(options: Options = {}) {
	const { onSendInvite, onRemove } = options;
	const columns = useMemo<ColumnDef<EnrolledProgramUserRow>[]>(
		() => [
			{
				accessorKey: "name",
				header: "NAME",
