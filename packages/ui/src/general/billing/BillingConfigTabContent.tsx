...
import { formatUtcLongDate } from "@repo/shared";
...
export const BILLING_CONFIG_SECTION_KEYS = [
	"general",
	"invoice-preferences",
	"timekeeping",
	"fee-structure",
...
export type BillingConfigSectionKey =
	(typeof BILLING_CONFIG_SECTION_KEYS)[number];

const ALL_SECTIONS = new Set<BillingConfigSectionKey>(
	BILLING_CONFIG_SECTION_KEYS,
);

function sectionVisible(
	visibleSections: ReadonlySet<BillingConfigSectionKey> | undefined,
	section: BillingConfigSectionKey,
): boolean {
...
export interface BillingConfigTabContentProps {
	...
	/** When set, only these configuration sections are rendered. */
	visibleSections?: readonly BillingConfigSectionKey[];
}
...
}: Readonly<BillingConfigTabContentProps>) {
	const visibleSections = visibleSectionsProp
		? new Set(visibleSectionsProp)
		: ALL_SECTIONS;
	const show = (section: BillingConfigSectionKey) =>
		sectionVisible(visibleSections, section);
	if (isLoading) {
		return (
														...
														<div className="flex items-center justify-between py-3">
															...
															<div className="flex items-center gap-3">
																<span className="text-muted-foreground/70">
																	{formatUtcLongDate(item.observedOn)}
																</span>
