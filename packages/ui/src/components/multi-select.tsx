...
>) {
	...
	const debouncedCheckOverflow = useDebouncedCallback(checkOverflow, {
		wait: 100,
	});
	...
	if (single && selectedValues.size > 0) {
		...
		return (
			<span className="min-w-0 overflow-hidden">{items.get(firstValue)}</span>
		);
	}
		...
		<>
			...
			<PopoverContent className="min-w-(--radix-popover-trigger-width) p-0">
				<Command {...props}>
					...
					<CommandList onScrollToBottom={onScrollToBottom}>
						{canSearch && (
							<CommandEmpty>
